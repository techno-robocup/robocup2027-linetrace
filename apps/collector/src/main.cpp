...
int main(int argc, char** argv) {
  ...
  net::PreviewSender preview(previewFd, args.previewPort, /*quality=*/55, /*fps=*/15.0);
  preview.start();
  ...
  auto onFrame = [&](const cv::Mat& bgr, uint64_t tsNs) {
    ...
    if (!rec) {
      // Recording stopped: close the session so the next REC starts a new one.
      std::lock_guard<std::mutex> lk(sessionMx);
      if (!activeSession.empty()) {
        writer.endSession();
        std::cerr << "session closed: " << activeSession << "\n";
        activeSession.clear();
      }
      return;
    }
